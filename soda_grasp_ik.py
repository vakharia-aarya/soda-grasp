...
def main(if_sim):
    ...
    # delta_q_error = np.dot(pseudo_inverse, delta_x)
    # print("delta_q_error")
    # print(delta_q_error)
    # print("arm_skeleton.get_positions()")
    # print(arm_skeleton.get_positions())
    # q = arm_skeleton.get_positions() - delta_q_error.T
    # print("q")
    # print(q)
    # # if type(q)!=list:
    # #     q=[1,2,3,4,5,6]
    ...
    # Second version slowly move the can
    delta_x = np.array([0.0,0.0,0.0,-0.01,0.0,0.0])        
    initial_pos = arm_skeleton.get_positions()
    while True:
        #delta_x[3] -= 0.01        
        if np.linalg.norm(arm_skeleton.get_positions() - initial_pos) >= 0.5:
            break
        jacobian = arm_skeleton.get_jacobian(hand.get_endeffector_body_node())
        # print("jacobian")
        # print(jacobian.shape)
